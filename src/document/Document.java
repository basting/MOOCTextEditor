...
public abstract class Document {
	...
	{
		// This method so that you can call it from the 
	    // getNumSyllables method in BasicDocument (module 1) and 
	    // EfficientDocument (module 2).
	    char[] letters = word.toCharArray();
	    int vowelCount = 0;
	    for(int i=0;i<letters.length;i++) {
	        char currC = letters[i];
	        if(isVowelSound(currC)) {
	            vowelCount++;
	            i = getNextNonVowelIdx(letters, i);
	        }
	    }
	    if(letters[letters.length-1] == 'e' 
	            && !isVowelSound(letters[letters.length-2])) {
	        if(vowelCount > 1) {
	            vowelCount--;
	        }
	    }
	...
	private int getNextNonVowelIdx(char[] letters, int startIdx) {
	    int i = startIdx;
	    while(i < letters.length && isVowelSound(letters[i])) {
	        i++;
	    }
	    return i;
	}
	
	private boolean isVowelSound(char c) {
	    switch(c) {
	        case 'a':
	        case 'A':
	        case 'e':
